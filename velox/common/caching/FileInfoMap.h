...
struct RawFileInfo {
  ...
  bool operator==(const RawFileInfo& other) {
    return openTimeSec == other.openTimeSec && cached == other.cached;
  }
};
...
class FileInfoMap {
 public:
  ...
  static const std::unique_ptr<FileInfoMap>& getInstance() {
    return instance_;
  }
  ...
  /// Reset the flag, cached, of RawFileInfo to false for all entries.
  void resetCached();

  /// Remove all entries with the RawFileInfo flag, cached, false.
  void deleteNotCached();
 ...
 private:
  ...
  static std::unique_ptr<FileInfoMap> instance_;
  static folly::SharedMutex fileMapMutex_;
